...
\begin{abstract}
Large language models used as automatic judges are known to be sensitive to surface cues that have nothing to do with answer quality---authorship hints, verbosity, declared peer consensus, emotional tone---yet the \emph{mechanism} by which such bias arises inside the judge remains unclear. We analyze LLM-as-a-judge scoring bias through the lens of activation geometry. Using tightly controlled triples of baseline, positively perturbed, and negatively perturbed answers across seven bias types, nine benchmarks, and seven judges, we first document a highly structured empirical landscape: negative perturbations induce substantial score degradation while positive ones are nearly inert, and the magnitude varies dramatically by bias type and input domain. At the representation level, baseline activations occupy a tight \emph{fair region}, while biased inputs are systematically displaced along a low-dimensional, type-specific bias direction that sharpens with depth and is recovered consistently by independent estimators. Bidirectional activation steering along this direction reproduces biased scoring (attack) and recovers fair scoring on biased inputs (defense), Pareto-dominating text-level interventions in Wasserstein shift and ordinal faithfulness---strong causal evidence that the direction carries the bias mechanistically. The same geometry supports a lightweight detector that predicts unfair judgments on unseen domains substantially better than a text-based baseline. Together, these results unify mechanism, intervention, and detection within a single activation-geometric framework for LLM judges.
\end{abstract}
...
\item A large-scale empirical characterization of LLM-as-a-judge scoring bias across $7$ bias types, $9$ benchmarks, and $7$ judges, revealing systematic asymmetry, type heterogeneity, and domain interaction (\Cref{subsec:landscape}).
\item An activation-geometric account of scoring bias: a fair region and a low-dimensional bias direction that emerges with depth and separates bias types (\Cref{subsec:geometry}).
\item A two-stage activation-steering procedure that provides symmetric causal control---Pareto-dominant attacks and defenses that together establish $\vec{v}_{\text{bias}}$ as the mechanistic carrier of bias (\Cref{subsec:causal}).
\item An activation-feature bias detector that outperforms text-based baselines, especially on unfairness prediction for unseen domains (\Cref{subsec:detection_brief}).
\end{itemize}
...
\begin{equation}
    \mathcal{D}_{\text{eff}}^{+} = \{(x_i^{\text{pos}}, x_i^{\text{base}}) : s(x_i^{\text{pos}}) - s(x_i^{\text{base}}) \geq \delta_s \}, \quad
...
\begin{equation}
\label{eq:alpha_obj}
\alpha^{*} \;=\; \argmax_{\alpha \in \mathcal{F}}\; W_1\!\left(s(\vec{h}'_l(\alpha)),\, s_{\text{target}}\right), \quad \mathcal{F} = \bigl\{\alpha : V(\alpha) \geq 0.95,\; \rho_S(\alpha) \geq 1.1\,\rho_S^{\text{text}} \bigr\},
\end{equation}
...
Our seven bias types are not introduced ad hoc; each is distilled from a distinct line of prior work on LLM-as-a-judge failure modes, and \Cref{tab:bias_construction} makes the provenance explicit alongside the operational transformation we use to instantiate each type. \citet{ye2024justice} catalogue a large family of surface biases---including emotional tone, metacognitive claims about revision, and social-identity cues---which directly motivate our \emph{Sentiment}, \emph{Refinement}, and \emph{Diversity} types. \citet{koo2024benchmarking} show that LLM evaluators inherit classical cognitive heuristics from social psychology, with bandwagon (majority-opinion) and authority effects among the most robust; these become our \emph{Bandwagon} and \emph{Authority} types. Self-preference and source-identity effects \citep{panickssery2024llm,zheng2023judging,wang2023large} are captured by \emph{Prestige}, and the long-documented length-favoring tendency \citep{saito2023verbosity,dubois2024length} by \emph{Verbosity}. Consolidating these threads into a single controlled suite lets us compare cognitive, stylistic, social, and source-attribution biases on the same footing---a prerequisite for the cross-bias geometry analyses in \Cref{subsec:geometry}. Every transformation modifies only surface framing and preserves the factual content and logical structure of the original answer, so that any resulting score shift is attributable to the bias attribute rather than a quality change; a human evaluation (\Cref{app:human_eval}) confirms that the two content-modifying types (Verbosity, Sentiment) do not degrade answer quality.

\begin{table*}[t]
    \caption{The seven bias types studied in this paper, their provenance in prior work on LLM-as-a-judge, and the operational transformations we apply to construct positively and negatively perturbed variants. Each transformation modifies only surface framing while preserving the factual content and logical structure of the original answer.}
    ...
    \begin{tabular}{p{1.8cm} p{3.3cm} p{4.3cm} p{4.3cm}}
        \toprule
        \textbf{Bias Type (source)} & \textbf{Description} & \textbf{Positive Construction} & \textbf{Negative Construction} \\
        \midrule
        \faStar\ \textbf{Prestige} \newline \citep{panickssery2024llm,zheng2023judging}
        ...
        \faAlignLeft\ \textbf{Verbosity} \newline \citep{saito2023verbosity,dubois2024length}
        ...
        \faUsers\ \textbf{Bandwagon} \newline \citep{koo2024benchmarking}
        ...
        \faFileSignature\ \textbf{Authority} \newline \citep{koo2024benchmarking,chen2024humans}
        ...
        \faTransgender*\ \textbf{Diversity} \newline \citep{ye2024justice,li2024split}
...
We scored all 4{,}500 baseline, 31{,}500 positively perturbed, and 31{,}500 negatively perturbed samples across our judge models and prompt configurations. The results reveal a striking and consistent asymmetry: \emph{negative perturbations induce substantial score degradation, while positive perturbations have negligible effect}. \Cref{tab:asymmetric_scoring} summarizes the results for \textsc{GPT-4.1} across all four prompt configurations. Negative perturbations consistently produce large, statistically significant score drops (Cohen's $d \in [-0.58, -0.35]$, all $p < 10^{-3}$), whereas positive perturbations yield negligible shifts (Cohen's $d \approx 0$, no significant $p$-values). This asymmetry persists regardless of whether CoT or stricter scoring criteria are used, ruling out prompting artifacts. Notably, the strict-scoring configurations---which lower the baseline mean from ${\sim}8.5$ to ${\sim}5.5$---eliminate the possibility that the absence of positive inflation is merely a ceiling effect. Results for all seven judge models under the default configuration are in \Cref{app:asymmetry_replication}; the pattern is consistent.

\begin{table*}[t]
\centering
\caption{Asymmetric scoring response to positive and negative perturbations across different prompt configurations for \textbf{GPT-4.1}. We report the baseline mean score ($\bar{s}_{\text{base}}$), mean score difference ($\Delta \bar{s}$), Cohen's d for effect size, t-test p-value, and 1-Wasserstein distance ($W_1$). Negative perturbations consistently induce a substantial penalty, while positive perturbations have a negligible effect. This asymmetry holds regardless of whether CoT or stricter scoring criteria are used.}
...
\Cref{fig:score_dist} shows the score distributions from \textsc{Llama-3.1-8B} under the default setting. The baseline mean is 5.84; positive perturbations yield a trivial increase to 5.91 (Cohen's $d = 0.048$, $p = .003$)---statistically significant due to sample size but practically negligible. In contrast, negative perturbations shift the mean down to 5.12 (Cohen's $d = -0.505$, $p < 10^{-189}$), reflecting a genuine distributional shift. LLM judges are substantially more vulnerable to negative cues than to positive flattery. This pronounced negativity bias is itself a central finding that shapes our subsequent analysis.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{images/score_distribution_Llama-3.1-8B.pdf}
\caption{Score distributions from \textsc{Llama-3.1-8B} (strict, non-CoT). The positive perturbation distribution ($\mathcal{D}_{\text{pos}}$, center) nearly overlaps with the baseline ($\mathcal{D}_{\text{base}}$, left), whereas the negative perturbation distribution ($\mathcal{D}_{\text{neg}}$, right) exhibits a substantial leftward shift.}
\label{fig:score_dist}
\end{figure}


\paragraph{Heterogeneity across bias types.}
Not all bias types are equally potent. \Cref{tab:bias_type_impact} breaks down the per-type impact for \textsc{Llama-3.1-8B}. On the negative side, biases targeting social-cognitive heuristics are far more effective: Bandwagon (suggesting peer disapproval) causes a 1.56-point drop, followed by Diversity and Authority. In contrast, Sentiment---merely introducing emotional language---has almost no effect. On the positive side, only Refinement (claiming the answer was ``professionally revised'') produces a notable boost; most other positive perturbations are ineffective. This asymmetry at the type level reveals a clear vulnerability hierarchy: LLM judges are particularly susceptible to social proof and credibility cues, and far more so in the negative direction.

\begin{wraptable}{r}{0.68\textwidth}
\vspace{-1em}
\centering
\caption{Impact of individual bias attributes on the mean score from \textsc{Llama-3.1-8B}. The baseline mean score is 5.84.}
\label{tab:bias_type_impact}
\begin{tabular}{l cc l cc}
\toprule
\multicolumn{3}{c}{\cellcolor{blue!10}\faThumbsUp[regular] \textbf{Positive Bias Attributes}} & \multicolumn{3}{c}{\cellcolor{red!10}\faThumbsDown[regular] \textbf{Negative Bias Attributes}} \\
\cmidrule(r){1-3} \cmidrule(l){4-6}
\textbf{Bias Type} & \textbf{Mean} & \textbf{Effect} & \textbf{Bias Type} & \textbf{Mean} & \textbf{Effect} \\
\midrule
...
An intriguing observation is that the positive and negative rankings are not mirror images: Bandwagon is the strongest negative bias but one of the weakest---even counterproductive---positive biases, while Refinement dominates on the positive side but ranks only fourth on the negative side. This suggests that positive and negative biases may engage fundamentally different internal mechanisms, a hypothesis we investigate through the activation analysis of \Cref{subsec:geometry}.


\paragraph{Domain-dependent vulnerability profiles.}
The interaction between bias type and question domain reveals further structure. \Cref{fig:bias_heatmap} presents heatmaps of the mean score change induced by each bias type on each source dataset, separately for positive biases (left) and negative biases (right).

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{images/bias_heatmap.pdf}
\caption{Mean score change by bias type and source dataset for \textsc{Llama-3.1-8B}. \textbf{(a)} Positive biases. \textbf{(b)} Negative biases. CSQA denotes CommonsenseQA. The interaction structure shows that vulnerability is not uniform---certain domains are differentially susceptible to specific bias types.}
\label{fig:bias_heatmap}
\end{figure}

Three patterns stand out. First, Bandwagon bias is universally potent in the negative direction across all domains, with particularly devastating effects on reasoning-heavy benchmarks (CommonsenseQA: $-2.46$, ARC-Challenge: $-2.13$). Second, Diversity bias shows domain sensitivity: its negative effect is strongest on socially relevant datasets such as SocialMaze and BBQ, consistent with these domains engaging social reasoning pathways. Third, Authority bias disproportionately affects knowledge-intensive domains (PubMedQA: $-1.25$, MMLU: $-0.98$), where citation signals carry greater weight. On the positive side, Refinement bias shows a remarkably consistent boosting effect across all domains, while other positive biases are domain-dependent and generally weaker. These domain-dependent profiles suggest that bias is not a monolithic phenomenon but reflects the interaction between surface cues and domain-specific processing, motivating the activation-level investigation that follows.


\subsection{Activation Geometry}
\label{subsec:geometry}

Having established the behavioral landscape, we now examine the judge's internal representations. Our goal is to characterize how biased inputs displace activations in hidden space, whether different bias types leave distinct geometric signatures, and how these signals evolve across the layer stack. Unless otherwise indicated, results below are for \textsc{Llama-3.1-8B}, with replications on \textsc{Qwen3-14B} and \textsc{Gemma-3-12B} in \Cref{app:geometric_replication}.

\subsubsection{Fair Region Discovery}
\label{subsec:fair_region}

We begin by visualizing the absolute positions of activations via Multidimensional Scaling (MDS). \Cref{fig:activation_mds_combined}a shows an MDS projection of final-token activations at layer 25 of \textsc{Llama-3.1-8B}, where each point is the hidden state $\vec{h}_{25}(x)$ for inputs from $\mathcal{D}_{\text{base}}$ and four representative bias types spanning both polarities (Bandwagon$-$, Diversity$-$, Refinement$+$, Sentiment$+$). A striking pattern emerges: baseline activations form a distinct, tight cluster, while the vast majority of biased activations---regardless of polarity---sit at a substantial distance from this baseline region. A small subset of biased activations remain anomalously close to the baseline cluster, clearly separated from the main biased mass; the same pattern holds at earlier layers (\Cref{app:visualizations}).

This clustering suggests that unbiased activations occupy a stable, well-defined region---a \emph{fair zone} from which biased inputs are systematically displaced. To verify that the spatial separation is not an artifact of other variables, we re-color the same MDS space by assigned score (\Cref{fig:activation_mds_combined}b) and by source dataset (\Cref{fig:activation_mds_combined}c). Neither score values nor dataset origins correlate with the observed clusters, confirming that the separation reflects the effect of bias on the model's internal state rather than a confound of scoring distribution or domain.
...
This phenomenon is not unique to MDS. PCA-based dimensionality reduction yields similar patterns, further validating that biased activations are displaced from a stable, unbiased region (\Cref{app:visualizations}). Together these observations establish a clear geometric structure: baseline activations occupy a coherent fair region from which biased inputs are displaced, motivating a closer examination of the \emph{directional} properties of that displacement.


\subsubsection{Directional Separation of Bias Types}
\label{subsec:directional_separation}

The fair-region analysis reveals \emph{that} biased activations are displaced, but does not distinguish \emph{how} different bias types displace them. We now turn to the effective bias vectors $\Delta \vec{h}_l(x)$ and ask whether different bias types produce geometrically distinct directions of displacement.

\Cref{fig:mds_bias_vectors} presents MDS projections of $\Delta \vec{h}_l$ for two contrasting bias types---Refinement (positive) and Diversity (negative)---at layers 5, 15, and 25 of \textsc{Llama-3.1-8B}. At layer 5, some distinction between the two populations emerges but remains weak. By layer 15 the separation is fairly clear, and by layer 25 the two types occupy fully distinct regions of the MDS space. This progressive separation demonstrates that different bias types---even across polarities---induce \emph{type-specific geometric signatures} in the model's internal representations, and that these signatures become increasingly distinguishable at deeper layers.

\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{images/effective_bias_vectors_ref_div_llama.pdf}
\caption{MDS projection of per-sample effective bias vectors $\Delta \vec{h}_l$ for Refinement (positive, orange) and Diversity (negative, blue) at layers 5, 15, and 25 of \textsc{Llama-3.1-8B}. The two bias types become increasingly separable in deeper layers, indicating type-specific geometric signatures in activation space.}
\label{fig:mds_bias_vectors}
\end{figure}

This geometric distinction is not limited to cross-polarity comparisons. MDS visualizations of two \emph{same-polarity} negative bias types (Bandwagon vs.\ Diversity) reveal the same progressive separation (\Cref{app:visualizations}), confirming that each bias type carves out its own geometric trajectory even within a single polarity. Consistent patterns are observed across architectures: \textsc{Gemma-3-12B} and \textsc{Qwen3-14B} exhibit analogous type-specific separation (\Cref{app:geometric_replication}).

Complementary analyses further characterize the structure of the bias-induced shift. PCA projections of the raw activations from $\mathcal{H}_{\text{base}}$, $\mathcal{H}_{\text{pos}}$, and $\mathcal{H}_{\text{neg}}$ show all three sets thoroughly intermingled along the principal components, which capture over 80\% of the variance (\Cref{app:visualizations}). This indicates that the bias-induced shift is \emph{orthogonal to the dominant content axes}: it is a subtle, non-principal direction specific to bias rather than a general ``low-quality'' signal.


\subsubsection{Layer-wise Characterization of the Bias Direction}
\label{subsec:layer_analysis}

The preceding geometric findings raise a quantitative question: how does the bias signal evolve across depth? We track four complementary scalars at each layer $l$ on the pooled negative-bias samples of \textsc{Llama-3.1-8B}:
\begin{itemize}[leftmargin=1.2em, nosep]
    \item \textbf{Biased core fraction} $\rho_{\text{core}}^{(l)} = |\mathcal{D}_{\text{far}}^{(l)}|/|\mathcal{D}_{\text{eff}}^{-}|$: the share of effective negative samples whose Mahalanobis distance from $\mathcal{M}_{\text{fair}}$ exceeds the 90th percentile of the baseline distance distribution.
    \item \textbf{LDA separability} $\text{Sep}_{\text{LDA}}^{(l)}$: leave-one-out accuracy of $\vec{v}_{\text{LDA}}^{(l)}$ in discriminating $\mathcal{H}_{\text{base}}^{(l)}$ from $\mathcal{H}_{\text{far}}^{(l)}$.
    \item \textbf{Classifier AUC} $\text{AUC}_{\text{CLS}}^{(l)}$: held-out AUC of the linear classifier trained to separate $\mathcal{H}_{\text{base}}^{(l)}$ from $\mathcal{H}_{\text{neg}}^{(l)}$ (the full, unfiltered negative set).
    \item \textbf{Direction agreement} $\cos\angle(\vec{v}_{\text{LDA}}^{(l)}, \vec{v}_{\text{CLS}}^{(l)})$.
\end{itemize}

\Cref{tab:layerwise} reports these four quantities at six representative layers. Three patterns are clear. \emph{(i)} The biased core fraction and LDA separability rise monotonically with depth (32\%/0.71 at layer 5 to 87\%/0.95 at layer 31), confirming that the fair region and the bias-displaced region become progressively better separated in deeper representations. \emph{(ii)} The cosine similarity between $\vec{v}_{\text{LDA}}$ and $\vec{v}_{\text{CLS}}$ climbs from 0.52 to 0.94: two direction estimators with entirely different objectives (Fisher's ratio versus a margin loss) converge on the same axis in deeper layers, indicating that the bias is concentrated in a low-dimensional subspace that emerges gradually through the network. \emph{(iii)} In contrast, the classifier AUC peaks at intermediate layers ($l \approx 10$--$15$) and declines slightly thereafter. We read this as a surface-vs-semantic tradeoff: early-to-middle layers retain trivially separable lexical cues distinguishing $\mathcal{D}_{\text{base}}$ from $\mathcal{D}_{\text{neg}}$, while deeper layers reorganize the geometry around a cleaner semantic bias axis captured by $\vec{v}_{\text{LDA}}$ and the biased core---a more selective but less surface-driven signal.

\begin{table}[h!]
\centering
\small
\caption{Layer-wise characterization of the bias direction on \textsc{Llama-3.1-8B} (32 layers, indexed 0--31). All quantities are computed on the pooled effective negative samples $\mathcal{D}_{\text{eff}}^{-}$ against $\mathcal{D}_{\text{base}}$. The biased core fraction, LDA separability, and LDA/Classifier cosine agreement increase monotonically with depth; the raw-classifier AUC peaks in middle layers.}
\label{tab:layerwise}
\begin{tabular}{c cccc}
\toprule
\textbf{Layer $l$} & \textbf{Biased Core $\rho_{\text{core}}^{(l)}$} & \textbf{LDA Sep. $\text{Sep}_{\text{LDA}}^{(l)}$} & \textbf{Classifier AUC} & $\cos\angle(\vec{v}_{\text{LDA}}, \vec{v}_{\text{CLS}})$ \\
\midrule
...
These observations directly inform the intervention experiments of the next subsection: the biased core gets cleaner and the bias subspace more agreed-upon in deeper layers, suggesting that intervention at mid-to-late layers should be most reliable. The $\vec{v}_{\text{LDA}}$--$\vec{v}_{\text{CLS}}$ convergence also foreshadows an empirical observation we exploit below: within each family, the three directional or three discriminative estimators agree strongly, allowing us to collapse six candidates to three representatives without loss of signal.

\paragraph{Magnitude versus direction.}
One could worry that the geometry above is conflated with a magnitude effect---that biased activations are simply ``larger'' than baseline ones. We rule this out directly. \Cref{fig:l2_norm} (\Cref{app:l2_norm}) plots the mean L2 norm $\|\vec{h}_l\|_2$ as a function of layer for $\mathcal{D}_{\text{base}}$ and $\mathcal{D}_{\text{neg}}$ on \textsc{Llama-3.1-8B}; the two curves track each other to within 3.5\% at every layer and a Welch's $t$-test fails to reject equality of means at $p>0.1$ everywhere. Bias is therefore encoded in the \emph{direction} of the activation displacement, not in its magnitude. This is why we unit-normalize all vector estimators before use, and why we define the biased core in terms of Mahalanobis distance rather than raw norm.
...
For each bias type we extract all six candidate vectors from the training set and compute pairwise cosine similarities on the layer-25 representations (full matrix in \Cref{tab:vector_cosine}). The three directional estimators agree tightly with $\cos \geq 0.93$, as do the three discriminative estimators with $\cos \geq 0.91$; cross-family agreement is lower ($\cos \in [0.55, 0.65]$), confirming that the two families capture genuinely distinct---but each internally coherent---notions of bias direction. We therefore retain three representatives spanning the spectrum: the \textbf{Geometric Median} (robust directional estimator), \textbf{PCA} (principal directional axis), and the \textbf{Classifier} vector (discriminative boundary).


\paragraph{Attacks.}
\Cref{tab:attack_summary} reports the best activation attack for three representative negative bias types on the held-out test set of \textsc{Llama-3.1-8B}, benchmarked against a text-based attack that directly applies the corresponding perturbation to the input. In every case the activation attack \emph{Pareto-dominates}: a strictly larger Wasserstein shift $W_1$ while also preserving a higher Spearman correlation $\rho_S$ with the baseline ranking. On Bandwagon$-$, for instance, the PCA attack more than doubles the shift (3.43 vs.\ 1.66) while raising $\rho_S$ from 0.51 to 0.58. Output validity stays above 95\% throughout, confirming that the binding constraint in practice is ordinal consistency rather than generation integrity.

\begin{table}[h!]
\centering
\small
\caption{Best-performing activation attack selected via \Cref{eq:alpha_obj} on the development set, evaluated on the test set of \textsc{Llama-3.1-8B}. Higher $W_1$ means stronger attack; higher $\rho_S$ means more ordinally faithful. Activation attacks Pareto-dominate text attacks on both axes across all three bias types. Full per-layer, per-method tables are in \Cref{app:attack_full}.}
\label{tab:attack_summary}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l l c c c c c}
\toprule
\textbf{Bias} & \textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.}\,$\uparrow$ & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
\multirow{2}{*}{Authority$-$} & Text Attack & 0.780 & 0.696 & 100\% & -- & -- \\
                              & \textbf{Classifier} (ours) & \textbf{1.618} & \textbf{0.780} & 94.8\% & 30 & 62.36 \\
\midrule
\multirow{2}{*}{Bandwagon$-$} & Text Attack & 1.664 & 0.514 & 100\% & -- & -- \\
                              & \textbf{PCA} (ours) & \textbf{3.426} & \textbf{0.584} & 94.8\% & 31 & 75.75 \\
\midrule
\multirow{2}{*}{Refinement$-$} & Text Attack & 0.594 & 0.757 & 100\% & -- & -- \\
                               & \textbf{PCA} (ours) & \textbf{1.132} & \textbf{0.800} & 100\% & 31 & 35.53 \\
\bottomrule
\end{tabular}
\end{table}

Four additional patterns across the full sweep warrant note (all detailed in \Cref{app:attack_full}):
\emph{(i) strength is highly heterogeneous}---optimal $\alpha^{*}$ varies by more than an order of magnitude across bias types and methods, so no single calibrated strength transfers;
\emph{(ii) depth matters}---$\alpha^{*}$ generally grows with layer depth, with a characteristic drop at the final layer where interventions disrupt ordinal logic more aggressively;
\emph{(iii) no single best estimator}---Classifier vectors typically yield the strongest push, Geometric Median vectors the most ordinally gentle, and PCA occasionally wins at deep layers, reinforcing our choice to retain one representative per family;
\emph{(iv) validity is rarely binding}---the 95\% validity constraint is active in $<10\%$ of configurations, meaning Spearman consistency, not output corruption, is the binding factor.

\paragraph{Defenses.}
Symmetrically, applying $\vec{h}'_l = \vec{h}_l - \alpha\,\vec{v}_{\text{bias}}^{(l)}$ to inputs in $\mathcal{D}_{\text{eff}}^{-}$---steering biased activations back toward the fair region---should recover baseline scores if $\vec{v}_{\text{bias}}$ is the mechanistic carrier of bias. Using the same two-stage search with $s_{\text{target}}$ set to the unperturbed baseline score, the activation defense reduces the Wasserstein distance between biased and baseline score distributions by 52--62\% across the three bias types (\Cref{tab:defense_summary}), compared to 9--15\% for a text-based rewrite defense targeted at the same attributes. This symmetric controllability---the judge can be pushed toward \emph{or} pulled away from the bias along the same direction---is the strongest causal evidence that $\vec{v}_{\text{bias}}$ is not merely correlated with biased scoring but causally drives it.

\begin{table}[h!]
\centering
\small
\caption{Best-performing activation defense on \textsc{Llama-3.1-8B}, test set. ``$W_1$-red.'' is the fractional reduction in Wasserstein distance between the defended and baseline score distributions relative to the pre-defense $W_1$. Activation defense recovers roughly half of the score degradation; text-rewrite defense recovers under 15\%. Full tables in \Cref{app:defense_full}.}
\label{tab:defense_summary}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l l c c c}
\toprule
\textbf{Bias} & \textbf{Method} & $W_1$-red.\,$\uparrow$ & $\rho_S$\,$\uparrow$ & \textbf{Valid.}\,$\uparrow$ \\
\midrule
\multirow{2}{*}{Authority$-$} & Text Rewrite & 0.11 & 0.74 & 100\% \\
                              & \textbf{Classifier} (ours) & \textbf{0.62} & \textbf{0.81} & 96.5\% \\
\midrule
\multirow{2}{*}{Bandwagon$-$} & Text Rewrite & 0.09 & 0.56 & 100\% \\
                              & \textbf{PCA} (ours) & \textbf{0.61} & \textbf{0.67} & 95.9\% \\
\midrule
\multirow{2}{*}{Refinement$-$} & Text Rewrite & 0.15 & 0.77 & 100\% \\
                               & \textbf{Classifier} (ours) & \textbf{0.52} & \textbf{0.83} & 97.1\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Dimension} & Mean $\pm$ Std & \% Degraded & Mean $\pm$ Std & \% Degraded \\
\midrule
Factual Consistency & 2.95 $\pm$ 0.23 & 1.7\% & 2.96 $\pm$ 0.21 & 1.3\% \\
Logical Coherence   & 2.91 $\pm$ 0.31 & 3.1\% & 2.93 $\pm$ 0.28 & 2.4\% \\
Completeness        & 2.87 $\pm$ 0.36 & 4.3\% & 2.94 $\pm$ 0.25 & 1.9\% \\
Overall Quality     & 2.81 $\pm$ 0.42 & 6.2\% & 2.84 $\pm$ 0.39 & 5.1\% \\
\midrule
Inter-annotator $\kappa$ & \multicolumn{2}{c}{0.73} & \multicolumn{2}{c}{0.69} \\
\bottomrule
\end{tabular}
\end{table}
...
\rowcolors{2}{gray!8}{white}
\begin{tabular}{p{1.7cm} p{6.6cm} p{6.6cm}}
\toprule
...
\begin{tabular}{l ccc}
\toprule
\textbf{Metric} & \textbf{Monotone} & \textbf{Non-monotone} & \textbf{Strict direction} \\
\midrule
$W_1(\alpha)$ (efficacy) & 97\% & 3\% & non-decreasing \\
$V(\alpha)$ (validity) & 99\% & 1\% & non-increasing \\
$\rho_S(\alpha)$ (Spearman) & 93\% & 7\% & non-increasing \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Attack performance for \textbf{Authority$-$} on \textsc{Llama-3.1-8B}, test set. Best per-metric values in bold.}
\label{tab:attack_authority}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 0.780 & 0.696 & 100\% & -- & -- \\
...
\centering
\small
\caption{Attack performance for \textbf{Bandwagon$-$} on \textsc{Llama-3.1-8B}, test set.}
\label{tab:attack_bandwagon}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 1.664 & 0.514 & 100\% & -- & -- \\
...
\centering
\small
\caption{Attack performance for \textbf{Refinement$-$} on \textsc{Llama-3.1-8B}, test set.}
\label{tab:attack_refinement}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 0.594 & 0.757 & 100\% & -- & -- \\
...
\centering
\small
\caption{Attack performance for \textbf{Diversity$-$} on \textsc{Llama-3.1-8B}, test set.}
\label{tab:attack_diversity}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 1.452 & 0.610 & 100\% & -- & -- \\
...
\centering
\small
\caption{Attack performance for \textbf{Verbosity$-$} on \textsc{Llama-3.1-8B}, test set.}
\label{tab:attack_verbosity}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 0.612 & 0.723 & 100\% & -- & -- \\
...
\centering
\small
\caption{Attack performance for \textbf{Refinement$+$} on \textsc{Llama-3.1-8B}, test set. This is the only positive bias in our attack suite; the activation push is in the score-up direction.}
\label{tab:attack_refinement_pos}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Method} & $W_1 \uparrow$ & $\rho_S \uparrow$ & \textbf{Valid.} & \textbf{Layer} & $\alpha^{*}$ \\
\midrule
Text Attack (baseline) & 0.382 & 0.812 & 100\% & -- & -- \\
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{images/l2_norm_layers.pdf}
...
\vspace{-1em}
\centering
\includegraphics[width=0.34\textwidth]{images/pca_activation.pdf}
...
\Cref{fig:pca_appendix} shows a PCA projection of final-token activations at layer 25 of \textsc{Llama-3.1-8B}, colored by bias type. Consistent with the MDS analysis in the main text (\Cref{fig:activation_mds_combined}), baseline activations form a tight cluster---the fair zone---while effective bias samples from different bias types are displaced away from this region. The agreement between MDS and PCA confirms that the fair zone is a robust geometric property of the activation space, not an artifact of a particular dimensionality reduction method.
