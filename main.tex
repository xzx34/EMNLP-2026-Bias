...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{images/score_distribution_Llama-3.1-8B.pdf}
...
MDS projections of final-token activations at layer 25 (\Cref{fig:activation_mds_combined}, \Cref{app:visualizations}) reveal that baseline activations form a tight cluster while the vast majority of biased activations---regardless of polarity---sit at a substantial distance from this region. Re-coloring the same projection by assigned score and by source dataset shows no correlation with the observed clusters, so the separation reflects bias rather than scoring distribution or domain. PCA yields the same picture, and the pattern holds at earlier layers. Unbiased activations therefore occupy a stable \emph{fair region} from which biased inputs are systematically displaced.
...
\section{Bias Landscape Details}
\label{app:landscape_details}

\Cref{tab:bias_type_impact} reports the per-type mean scores for \textsc{Llama-3.1-8B} (baseline mean 5.84) referenced in \Cref{subsec:landscape}, and \Cref{fig:bias_heatmap} shows the per-type, per-domain heatmap.
...
\subsection{Fair Region at Layer 25}

\Cref{fig:activation_mds_combined} shows the MDS projection underlying the fair-region discussion in \Cref{subsec:fair_region}: at layer 25, baseline activations form a tight cluster while the vast majority of biased activations---across polarities---are displaced. Re-coloring by score (b) or source dataset (c) shows no correlation with the clusters, confirming that the separation reflects bias rather than scoring distribution or domain.
...
\subsection{Cross-Polarity Geometric Separation}

\Cref{fig:mds_bias_vectors} presents MDS projections of $\Delta \vec{h}_l$ for two contrasting bias types---Refinement (positive) and Diversity (negative)---at layers 5, 15, and 25. The two populations are weakly distinguished at layer 5, more clearly separated at layer 15, and fully distinct at layer 25, confirming that different bias types induce type-specific geometric signatures that sharpen with depth.
